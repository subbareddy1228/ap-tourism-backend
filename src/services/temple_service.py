...
class TempleService:
    ...
    def _delete_cache(self, *keys: str):
        if not self.redis:
            return
        try:
            for key in keys:
    ...
    async def get_featured(self) -> List[TempleListItem]:
        ...
        self._set_cache(cache_key, [r.model_dump() for r in result], ttl_seconds=3600)
        return result
    ...
    async def get_popular(self, limit: int = 10) -> List[TempleListItem]:
        ...
        result  = [TempleListItem.model_validate(t) for t in temples]
        self._set_cache(cache_key, [r.model_dump() for r in result], ttl_seconds=1800)
        return result
    ...
    async def get_temple_detail(self, temple_id: UUID) -> TempleDetail:
        ...
        self._set_cache(cache_key, result.model_dump(), ttl_seconds=900)
        return result
    ...
    async def create_temple(self, data: TempleCreate) -> TempleDetail:
        ...
        self._delete_cache("temples:featured", "temples:popular:10")
