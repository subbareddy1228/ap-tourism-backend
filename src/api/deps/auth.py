...
# ───────── LIGHTWEIGHT USER ID HELPERS ─────────
 
async def get_current_user_id(
    credentials: HTTPAuthorizationCredentials = Depends(bearer_scheme),
) -> str:
    """
    Returns just the user_id (sub) from the JWT token.
    Lighter than get_current_user — no DB query.
    Used by endpoints that only need to know who the user is, not their full record.
    """
    if credentials is None:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
...
async def get_optional_user_id(
    ...
    """
    Returns user_id if a valid token is provided, otherwise None.
    Use for endpoints that work for both guests and logged-in users (e.g. search).
    """
    if credentials is None:
        return None
    payload = decode_token(credentials.credentials)
